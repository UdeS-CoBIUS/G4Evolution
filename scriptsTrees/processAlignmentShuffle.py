			...
			if w[0] != '':
				if w[4] == gtf['Gene']:
					...
					dicoCoordpG4[pG4coord] = {'CoordAlignment': [0, 0],
						...
						'Chromosome': gtf['Chr'], 'Strand': gtf['Strand']}
				# else:
				# 	print(w)
	# pprint(dicoCoordpG4)
...
def readAlignment(alignmentFile, dicoGeneList, path, tree):
	...
	for record in alignment :
		...
		pG4File = path + 'pG4Shuffle/'+ tree
		gtf[gene] = readGTF(gtfFile, gene)
		cpt += 1
		if gene not in pG4s:
			pG4s[gene] = []
		pG4tmp, dicoCoordpG4 = readpG4(pG4File, gtf[gene])
		for pG4 in dicoCoordpG4:
			dicoCoordpG4[pG4]['Species'] = sp
...
def main(path, tree):
	...
	df = pd.DataFrame()
	alignmentFile = path + 'AlignmentOrthoShuffle/'+tree
...
def build_arg_parser():
	parser = argparse.ArgumentParser(description = 'processAlignmentShuffle')
	GITDIR = os.getcwd()+'/'
